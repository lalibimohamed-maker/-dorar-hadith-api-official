...
const server = http.createServer(async (req, res) => {
  ...
  const locale = requestedLanguage ? localeFromRequest(requestedLanguage) : (detectLocale(queryText) || localeFromRequest(req.headers["accept-language"] || DEFAULT_LOCALE));
  ...
  if (url.pathname === "/") return sendJson(res, 200, { name: "موسوعة دين الله", nameEn: "Deen Allah Encyclopedia", service: "Deen Allah API", version: API_VERSION, locale, direction: locale.dir, endpoints: { health: "/health", portal: "/portal", portalCapability: "/portal/capability?id=quran", portalCapabilities: "/portal/capabilities", portalPolicy: "/portal/policy", locales: "/locales", search: "/search?q=...", quranAyah: "/quran/ayah?verse=1:1&translationIds=...&tafsirIds=...", quranTranslations: "/quran/translations?lang=en", sources: "/sources", books: "/books", authors: "/authors", categories: "/categories", maqasid: "/maqasid", tajweed: "/tajweed", tajweedLesson: "/tajweed/lesson?id=letters", fiqh: "/fiqh", fiqhResearch: "/fiqh/research?q=...", fiqhTemplate: "/fiqh/template?q=...", domains: "/research/domains", domain: "/research/domain?id=tafsir", scholars: "/research/scholars?q=...&domain=aqeedah", hadithMethodology: "/hadith/methodology", narratorGrades: "/hadith/narrator-grades", chainPhenomena: "/hadith/chain-phenomena", rijalBooks: "/hadith/rijal-books", narratorProfile: "/hadith/narrator/profile?name=...", narratorCompare: "/hadith/narrator/compare", inheritance: "/inheritance?estate=100000&sons=1&daughters=1&madhhab=hanbali", inheritanceMadhahib: "/inheritance/madhahib", inheritanceComplexCases: "/inheritance/complex-cases" } });
  if (url.pathname === "/portal") return sendJson(res, 200, { locale, direction: locale.dir, portal: getKnowledgePortal() });
  if (url.pathname === "/portal/capabilities") return sendJson(res, 200, { locale, capabilities: listKnowledgeCapabilities() });
  if (url.pathname === "/portal/capability") { const id = requireString(url.searchParams.get("id")); if (!id) return sendJson(res, 400, { error: "Missing required query parameter: id" }); const capability = getKnowledgeCapability(id); return capability ? sendJson(res, 200, { locale, capability }) : sendJson(res, 404, { error: "Portal capability not found" }); }
  if (url.pathname === "/portal/policy") return sendJson(res, 200, { locale, policy: portalSourcePolicy() });
  ...
  if (url.pathname === "/fiqh/research") { const q = requireString(url.searchParams.get("q")); if (!q) return sendJson(res, 400, { error: "Missing required query parameter: q" }); return sendJson(res, 200, { locale, query: q, results: searchFiqhResearch(q), method: getFiqhResearchFramework().analysisSchema }); }
  if (url.pathname === "/fiqh/template") { const q = requireString(url.searchParams.get("q")); if (!q) return sendJson(res, 400, { error: "Missing required query parameter: q" }); return sendJson(res, 200, { locale, template: buildFiqhResearchTemplate(q, { madhhab: requireString(url.searchParams.get("madhhab")) }) }); }
