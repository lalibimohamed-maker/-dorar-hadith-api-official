        ...
        if code == 0:
            for line in out.splitlines():
                if line.lower().startswith('pages:'):
                    try: rec['pages'] = int(line.split(':', 1)[1].strip())
                    except ValueError: pass
    if tools['pdftotext']:
        code, out = run([tools['pdftotext'], '-f', '1', '-l', '3', '-layout', str(path), '-'])
        if code == 0: rec['has_text'] = bool(out.strip())
...
def process_ocr(rec, pdf_root, processed_root, tools):
    if rec['classification'] != 'scanned-pdf': return
    if not tools['ocrmypdf'] or not tools['tesseract']:
        ...
        return
    src = Path(rec['path']); rel = src.relative_to(pdf_root); dst = processed_root / rel
    ...
    if code != 0: dst.unlink(missing_ok=True)
...
def main():
    ...
    args = ap.parse_args()
    root = Path(args.root).resolve(); pdf_root = (root / args.pdf_root).resolve(); out = (root / args.out).resolve(); processed = (root / args.processed_root).resolve()
    out.parent.mkdir(parents=True, exist_ok=True)
    names = ['qpdf', 'pdfinfo', 'pdftotext', 'ocrmypdf', 'tesseract']; tools = {n: tool(n) for n in names}
    files = sorted(pdf_root.rglob('*.pdf')) if pdf_root.exists() else []; records = []
    for path in files:
        try:
            rec = inspect_pdf(path, tools)
            if args.ocr: process_ocr(rec, pdf_root, processed, tools)
            records.append(rec)
        except Exception as exc: records.append({'path': str(path), 'status': 'inspection-error', 'error': str(exc)})
    counts = {}
    for rec in records:
        key = rec.get('classification', rec.get('status', 'unknown')); counts[key] = counts.get(key, 0) + 1
    ocr_counts = {}
    for rec in records:
        key = rec.get('ocr', {}).get('status', 'not-run'); ocr_counts[key] = ocr_counts.get(key, 0) + 1
    manifest = {'schema':'din-allah-encyclopedia/rechercher-acquisition-engine-v2/v1','generated_at':datetime.now(timezone.utc).isoformat(),'policy':{'acquisition_is_primary':True,'originals_are_immutable':True,'ocr_is_authoritative':False,'processing_failure_is_non_blocking':True},'root':str(pdf_root.relative_to(root)) if pdf_root.is_relative_to(root) else str(pdf_root),'tools':tools,'counts':counts,'ocr_counts':ocr_counts,'records':records}
