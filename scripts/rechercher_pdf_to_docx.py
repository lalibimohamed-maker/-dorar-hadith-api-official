...
def main():
    ...
    for pdf in sorted(inp.rglob("*.pdf")):
        ...
        try:
            ...
            e.update(pdf_kind=kind,source_char_count=digital_chars,derived_char_count=derived_chars,derived_digital_char_count=len(derived_digital),page_statistics=pstats,vector_fallback_pages=sum(1 for x in pstats if x.get("vector_fallback")),tessdata=tessdata,table_metrics=table_metrics)
            if digital_chars:
