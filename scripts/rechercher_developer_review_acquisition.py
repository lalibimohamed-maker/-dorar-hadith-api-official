...
def main():
    ...
    for book in cat['books']:
        ...
        rec['rights_action']='public-eligible' if found and book.get('rights_status')=='verified-redistributable' else ('developer-review-pdf' if found else 'none')
        records.append(rec)
    summary={'schema':'developer-review-acquisition/v2','scope':cat['scope'],'principle':'catalog completeness is independent from redistribution rights; a technically valid PDF is not counted as acquired until bibliographic identity is verified; the original .pdf is the sole primary book representation and no new .pdf.enc duplicate is created','records':records,'counts':{'books':len(records),'acquired':sum(r['availability']=='copy-acquired' for r in records),'rate_limited':sum(r['acquisition_state']=='rate-limited' for r in records),'identity_unverified':sum(r['acquisition_state']=='identity-unverified' for r in records),'download_failed':sum(r['acquisition_state']=='download-failed' for r in records),'not_found':sum(r['acquisition_state']=='global-search-no-match' for r in records)}}
