...
def classify(entry):
    ...
    explicit_acq = any(
        r.get("acquisition") in (True, "true") or
        r.get("acquisition_capability") in (True, "true") or
        "pdf" in [str(x).lower() for x in r.get("capabilities", [])] or
        "pdf-download" in [str(x).lower() for x in r.get("capabilities", [])] or
        "pdf_downloads" in [str(x).lower() for x in r.get("capabilities", [])]
        for r in records
    )
    ...
    if explicit_acq or "pdf" in kinds or "book/pdf acquisition" in roles or "digital-copy/pdf acquisition" in roles:
        cls = "pdf_acquisition"
    elif any(x in roles or x in blob for x in ("manuscript", "digital scan", "digitized manuscript", "complete scan")):
        cls = "digital_scan_acquisition"
    elif any(x in roles or x in blob for x in ("catalogue", "bibliographic", "manuscript discovery", "provenance", "citation")):
        cls = "discovery_only"
    elif "api" in kinds or any(r.get("runtime") in ("api_key", "public-api", "public_api", "api_in_development") for r in records):
        cls = "api_evidence"
    else:
        cls = "content_review"
    return cls
    ...
    for entry in sorted(sources.values(), key=lambda x: x["id"]):
        report["sources"].append({
            ...
            "acquisition_explicit": any(
                r.get("acquisition") in (True, "true") or
                r.get("acquisition_capability") in (True, "true")
