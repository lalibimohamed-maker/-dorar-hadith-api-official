...
for (const record of rows) {
  ...
  if (DISCOVER) {
    try { analysis = await globalSearchAnalysts(record); }
    catch (error) { discoveryError = error instanceof Error ? error.message : String(error); }
  }
  const selected = analysis.selected ?? null;
  routed.push({ id: record.id, title: record.title ?? record.titleHint ?? null, author: record.author ?? null, originalSource: record.sourceUrl ?? record.sourcePage ?? null, originalRightsDecision: record.rightsDecision ?? 'unclear', acquisitionOutcome: selected ? 'use-alternative' : analysis.outcome, selectedAlternative: selected, candidates: analysis.candidates, providerStatus: analysis.providerStatus, discoveryAttempted: DISCOVER, discoveryError, originalRemainsDiscoverable: true, readOnlyFallback: !selected });
}
