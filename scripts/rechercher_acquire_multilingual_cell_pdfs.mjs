...
function markAlreadyAcquired(row){
  manifest.cells[row.cell_id]={
    ...
    language_iso:row.language_iso||null,
    domain:row.domain,
  ...
  for(const seed of seeds){
    ...
    for(const candidate of candidates){
      ...
      try{
        ...
        const durableDuplicate=existingSha.has(r.sha256);
        if(durableDuplicate){
        ...
        try{
          provenanceId=recordAcquisition(provDoc,{
            ...
            cellId:row.cell_id,
            sourceId,
            sourceLabel:(sourceWorldwide?.name||sourceMaster?.name||sourceAdapter?.name||sourceId),
