...
def candidate_urls(b):
 ...
 for s in [b.get('waqfeya_url')]+b.get('sources',[]):
  if not s:continue
  if s.endswith('.pdf'):out.append((s,s));continue
  try:
   for u in pdfs(fetch(s),s):out.append((s,u))
  except Exception:continue
 seen=set();result=[]
 for label,u in out:
  if u not in seen:seen.add(u);result.append((label,u))
 return result
def acquire_volume(b,n,candidates,work):
 ...
 for label,u in candidates:
  ...
  except Exception as e:attempts.append({'source':label,'url':u,'result':'failed','error':str(e)})
...
def acquire(b):
 ...
 work.mkdir(parents=True);vols=[acquire_volume(b,n,candidates,work) for n in range(1,exp+1)]
 unified=ART/f'{safe}.pdf';unified.unlink(missing_ok=True);pages=[str(work/f'{n:03d}.pdf') for n in range(1,exp+1)]
