...
import {
  ...
  rankSearchResults,
} from '../src/multi-source-search.js';
...
test('search providers are pluggable without changing corpus logic', async () => {
  registerSearchProvider({
    ...
    search: async ({ text }) => [{
      url: 'https://example.org/book/#section',
      title: text,
      sourceType: 'official',
  ...
  assert.equal(results[0].provenance.discoveryProvider, 'test-provider');
});
...
test('ranking never treats public discovery as redistribution permission', () => {
  const ranked = rankSearchResults([
    { url: 'https://example.org/a', sourceType: 'official', redistributionAllowed: false, confidence: 1 },
    { url: 'https://example.org/b', sourceType: 'secondary', redistributionAllowed: true, confidence: 0.1 },
  ]);
