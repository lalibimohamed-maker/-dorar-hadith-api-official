...
test("canonical corpus source ids are covered by the runtime source registry", () => {
  ...
  const missing = corpus.records.map((record) => record.sourceId).filter((sourceId) => !registryIds.has(sourceId));
...
test("canonical corpus source types are covered by the provenance policy", () => {
  ...
  const unsupported = [...new Set(corpus.records.map((record) => record.sourceType).filter((sourceType) => !policy.sourceTypes.includes(sourceType)))];
...
test("every canonical corpus source resolves through getSource", () => {
  const unresolved = corpus.records.filter((record) => !getSource(record.sourceId)).map((record) => record.sourceId);
