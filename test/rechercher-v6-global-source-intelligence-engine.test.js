...
test('V5 research output can hand off to V6 without bypassing the contract', () => {
  const registry = createStageNetworkRegistry();
  const v5 = createStageNodeContract({
    stageId: 'V5_GLOBAL_RESEARCH', version: '1.0',
    capabilities: ['GLOBAL_RESEARCH'],
    acceptedInputs: [{ type: 'STAGE_OUTPUT' }],
    producedOutputs: [{ type: 'RESEARCH_OUTPUT' }],
  ...
  const v6 = createV6StageNodeContract();
  registerNode(registry, v5);
  registerNode(registry, v6);
...
test('V6 discovers, classifies, resolves identity, evaluates rights and verifies a source', () => {
  ...
  const original = {
    ...
    canonicalQuranArabic: 'UNCHANGED',
    originalPdf: 'original.pdf',
    provenance: { retrieval: 'catalog-record', chain: ['catalog', 'repository'] },
...
test('V6 allows publication only when rights are explicitly ALLOWED', () => {
  const engine = createV6GlobalSourceIntelligenceEngine();
  discoverSource(engine, {
    candidateId: 'candidate-allowed', sourceUrl: 'https://example.org/open',
    contentHash: 'sha256:open', provenance: { retrieval: 'official-open-record' }, rightsState: 'UNKNOWN'
  });
...
test('V6 deduplicates the same source without replacing its immutable identity', () => {
  const engine = createV6GlobalSourceIntelligenceEngine();
  const base = {
    sourceUrl: 'https://example.org/same', contentHash: 'sha256:same',
    provenance: { retrieval: 'official' }, rightsState: 'ALLOWED'
  };
...
test('V6 links verified sources to work, edition and manuscript identities', () => {
  const engine = createV6GlobalSourceIntelligenceEngine();
  discoverSource(engine, {
    candidateId: 'candidate-link', sourceUrl: 'https://example.org/manuscript',
    provenance: { retrieval: 'manuscript-catalog' }, rightsState: 'EXPLICIT_PERMISSION_REQUIRED'
  });
  ...
  const link = linkSourceToWorkEditionManuscript(engine, 'source:manuscript:1', {
    workId: 'work:1', editionId: 'edition:1', manuscriptId: 'ms:1'
  });
  assert.deepEqual(link, {
    sourceId: 'source:manuscript:1', workId: 'work:1', editionId: 'edition:1', manuscriptId: 'ms:1', at: link.at
  });
