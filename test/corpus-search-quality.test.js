...
test('verifiedOnly excludes pending records', () => {
  const result = searchCorpus('الصلاة', { verifiedOnly:true }, records);
  assert.equal(result.results.some(item => item.id === 'book-fajr'), false);
});
