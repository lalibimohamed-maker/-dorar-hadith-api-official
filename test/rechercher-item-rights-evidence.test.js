...
test('rights extractor captures the complete required metadata contract', () => {
  assert.deepEqual(RIGHTS_FIELDS, [
  ...
  const evidence = extractRightsMetadata({
    itemId: 'IA:book-123',
    resourceId: 'res-123',
    sourceUrl: 'https://example.org/item/123',
    metadata: {
      identifier: 'IA:book-123',
      license: 'CC BY-SA 4.0',
      license_url: 'https://creativecommons.org/licenses/by-sa/4.0/',
      rights: 'May be redistributed with attribution and share-alike.',
      copyright: 'Public domain notice',
      public_domain: true,
      cc0: false,
      cc_by: true,
      cc_by_sa: true,
      permission_statements: 'Permission to redistribute with attribution.',
      institutional_terms: 'Institutional terms apply.'
    }
  });
...
test('generic source rights do not become item-level redistribution permission', () => {
  const evidence = extractRightsMetadata({
    itemId: 'IA:book-123',
    sourceUrl: 'https://archive.org/legal/terms.php',
    text: 'Terms of use. Some materials may be public domain. CC BY resources may exist.'
  });
...
test('missing identity or ambiguous rights remain discoverable but fail closed', () => {
  const evidence = extractRightsMetadata({
    sourceUrl: 'https://example.org/item/123',
    text: 'CC BY 4.0 license may apply.'
  });
  const decision = decideItemRedistribution(evidence);
  assert.equal(decision.redistribution_permission, 'not-granted');
