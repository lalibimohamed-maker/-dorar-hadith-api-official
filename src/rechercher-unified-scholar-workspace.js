...
export function submitWorkspaceReview(workspace, { reviewId, linkId, reviewerId, reviewerRole, verdict, notes = '' } = {}) {
  ...
  const link = workspace.links.get(linkId);
  if (!link) throw new Error(`Unknown link: ${linkId}`);
  const review = { reviewId, linkId, reviewerId, reviewerRole, verdict, notes };
  workspace.reviews.set(reviewId, review);
  const session = workspace.sessions.get(link.sessionId);
  if (verdict === 'APPROVED') {
    link.reviewRequired = false;
    session.state = 'ACTIVE';
  } else {
    session.state = verdict === 'REJECTED' ? 'PUBLISH_BLOCKED' : 'REVIEW_REQUIRED';
  }
  return review;
}
...
export function workspacePublishable(workspace, sessionId) {
  ...
  return links.every((link) => !link.reviewRequired && link.sourceIds.length > 0);
}
