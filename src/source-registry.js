...
const russianExpansion = readJson("russian-sources-2026.json", { locale: "ru", sources: [], policy: {}, routing: {} });
...
const russianSources = (russianExpansion.sources || []).map((source) => mapSource(source, { category: "russian", language: source.language || "ru", sourceKind: source.role || "russian-source", reusePolicy: source.downloadPolicy || "navigation_first" }));
const categories = [...registry.categories, { id: "fatwa", nameAr: "الفتاوى ومصادر العلماء" }, { id: "arabic-language", nameAr: "اللغة العربية والمعاجم والمصطلحات" }, { id: "russian", nameAr: "المصادر الإسلامية الروسية" }, { id: "history", nameAr: "التاريخ والأخبار" }, { id: "companions", nameAr: "أخبار الصحابة وتراجمهم" }, { id: "genealogy", nameAr: "الأنساب والقبائل" }, { id: "hadith-sciences", nameAr: "علوم الحديث ونقد الرواية" }, { id: "scholars", nameAr: "مصادر العلماء" }, { id: "research", nameAr: "مراكز البحوث والدراسات" }, { id: "academic", nameAr: "الجامعات والمؤسسات الأكاديمية" }, { id: "manuscripts", nameAr: "المخطوطات والفهارس" }].filter((category, index, all) => all.findIndex((item) => item.id === category.id) === index);
const mergedSourceCandidates = [...registry.sources, ...fatwaSources, ...saudiFatwaSources, ...secondaryFatwaSources, ...scholarSources, ...expandedSources, ...officialSources, ...academicSources, ...languageSources, ...russianSources];
