...
function fiqhMatches(query) {
  return searchFiqhResearch(query).map((item) => ({ ...item, topic: "fiqh", source: item.source || "https://shamela.ws/", work: item.title, methodology: item.author, verification: item.verification || item.status || "bibliographic-record", rights: "catalog-and-link-unless-licensed", corpus: "sunni" }));
}
...
export async function unifiedSearch(query, { signal, includePotentialMatches = false, responseLocale = "ar" } = {}) {
  ...
  const [hadithData, sourceMatches] = await Promise.all([
    ...
    Promise.resolve(searchUnified(query, records, { includePotentialMatches, corpus: "sunni", requireSource: true })),
  ]);
