...
export function registerSearchProvider({ id, name, search, languages = [], capabilities = [] }) {
  if (!id || !name || typeof search !== 'function') throw new Error('Search provider requires id, name and search()');
...
export function listSearchProviders() {
  return [...providers.values()].map(({ id, name, languages, capabilities }) => ({ id, name, languages: [...languages], capabilities: [...capabilities] }));
}
...
function normalizeCandidate(candidate, provider, query) {
  ...
  return { ...candidate, url: canonicalizeUrl(candidate.url), provider, query,
    rightsStatus: candidate.rightsStatus ?? 'rights-unclear', redistributionAllowed: candidate.redistributionAllowed === true,
    provenance: { ...(candidate.provenance ?? {}), discoveryProvider: provider, discoveryQuery: query } };
}
...
export function mergeSearchResults(results = []) {
  ...
  for (const result of results) {
    ...
    if (!previous || Number(normalized.redistributionAllowed) > Number(previous.redistributionAllowed)) byUrl.set(normalized.url, normalized);
  }
  return [...byUrl.values()];
}
...
export async function discoverAcrossProviders(query, { providerIds, languages, variants } = {}) {
  ...
  const results = [];
  for (const provider of selected) for (const searchQuery of queries) {
...
export function rankSearchResults(results = []) {
  ...
  return [...results].sort((a, b) => {
    const rights = Number(b.redistributionAllowed) - Number(a.redistributionAllowed); if (rights) return rights;
    const source = (sourceRank[b.sourceType] ?? 0) - (sourceRank[a.sourceType] ?? 0); if (source) return source;
