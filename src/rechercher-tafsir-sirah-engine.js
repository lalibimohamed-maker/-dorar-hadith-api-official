const DOMAINS = Object.freeze(['TAFSIR', 'SIRAH']);

function createTafsirSirahEngine() {
  return { works: new Map(), passages: new Map(), links: new Map(), disagreements: new Map() };
}

function registerWork(engine, { workId, domain, title, author, sourceId, sourceHash, rightsStatus = 'UNKNOWN' }) {
...
function registerPassage(engine, { passageId, workId, location, text, page = null }) {
...
function linkEvidence(engine, { linkId, passageId, claimId, relation = 'SUPPORTS' }) {
...
function recordDisagreement(engine, { disagreementId, claimId, passageIds, summary }) {
...
function publishableWork(engine, workId) {
  const work = engine.works.get(workId);
  return Boolean(work && work.rightsStatus === 'ALLOWED');
}

module.exports = { DOMAINS, createTafsirSirahEngine, registerWork, registerPassage, linkEvidence, recordDisagreement, publishableWork };
