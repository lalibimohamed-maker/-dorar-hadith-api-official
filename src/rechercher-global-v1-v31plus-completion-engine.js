...
export function createGlobalStageMatrix({ stages = [], capabilityCoverage = {} } = {}) { const normalized = stages.map(createStageRequirement); const matrix = normalized.map(stage => ({ ...stage, capabilityStatuses: Object.fromEntries(stage.capabilities.map(c => [c, normalizeStatus(capabilityCoverage[c])])) })); return { stageCount: matrix.length, familyCounts: Object.fromEntries(Object.keys(STAGE_FAMILIES).map(f => [f, matrix.filter(s => s.family === f).length])), stages: matrix }; }
