...
export function probeBinary(command) {
  ...
  return {
    ...
    command,
    version: result.status === 0 ? String(result.stdout || '').split(/\r?\n/, 1)[0] : null
  };
}
...
export function engineInventory() {
  return {
    ...
    realesrgan: probeBinary('realesrgan-ncnn-vulkan'),
    video2x: probeBinary('video2x')
  };
}
...
export function buildDerivedManifest({ inputPath, outputPath, mediaType, profile, engine }) {
  return {
    ...
    profile,
    engine,
    input: { path: inputPath, authoritative: true },
    output: { path: outputPath, authoritative: false },
    policy: {
      preserveOriginal: true,
      verifyBeforePromotion: true,
      noSilentTrustedReplacement: true
    },
...
export function validateTargetDimensions(width, height, requested) {
  ...
  if (target.maxLongEdge !== null && Math.max(width, height) > target.maxLongEdge) {
    return { allowed: false, reason: 'input-already-exceeds-profile' };
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command === 'inventory') {
    console.log(JSON.stringify(engineInventory(), null, 2));
    process.exit(0);
  }
  if (command === 'profile') {
    const [width, height, profile = '8k'] = process.argv.slice(3);
    const result = selectImageProfile(Number(width), Number(height), profile);
