...
function loadCatalog() {
  if (!cache) cache = JSON.parse(fs.readFileSync(FILE, 'utf8'));
  return cache;
}
...
export function resolveGhaybDomain(text) {
  ...
  if (!best) return null;
  return {
    id: `ghayb:${best.domain.id}`,
    type: 'ghayb-domain',
    domain: 'aqidah',
    ghaybDomain: best.domain.id,
    title_ar: best.domain.title_ar,
    matchedAlias: best.matchedAlias,
    score: best.score,
    creedSensitive: Boolean(best.domain.creedSensitive),
...
export function buildGhaybResearchPlan(text, options = {}) {
  ...
  return {
    ...
    stages: [
      ...
      'retrieve-quran',
      'retrieve-authenticated-sunnah',
      'retrieve-sound-early-reports',
      'retrieve-attributed-scholarly-works',
      'verify-citations-and-report-grading',
      'separate-fact-from-disputed-report',
      'answer-in-query-language'
    ],
    rules: {
      citeEveryClaim: true,
      showSourceLocator: true,
      showHadithGradeWhenAvailable: true,
      labelScholarOpinions: true,
      doNotPresentUnknownAsFact: true,
      comparativeCreedOffByDefault: !options.comparative
    }
  };
}

export function listGhaybDomains() {
  return (loadCatalog().domains || []).map(({ id, title_ar, aliases, creedSensitive }) => ({
    id, title_ar, aliases, creedSensitive: Boolean(creedSensitive)
  }));
}
