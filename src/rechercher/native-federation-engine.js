...
export async function federatedSearch(query, options = {}) {
  ...
  const perSource = await mapLimit(sources, options.concurrency ?? 5, async (source) => {
    ...
    try {
      ...
      return { sourceId: source.id, priority: priorityIds(options).has(source.id), status: 'success', records, elapsedMs: Date.now() - sourceStarted };
    } catch (error) {
      return { sourceId: source.id, priority: priorityIds(options).has(source.id), status: 'error', records: [], error: error.message, elapsedMs: Date.now() - sourceStarted };
    }
  });
