...
export async function federatedSearch(query, options = {}) {
  const sources = (options.sources ?? NATIVE_SOURCES).filter((s) => s.enabled !== false);
  ...
  const perSource = await mapLimit(sources, options.concurrency ?? 5, async (source) => {
    ...
    try {
      ...
      return { sourceId: source.id, status: 'success', records, elapsedMs: Date.now() - sourceStarted };
    } catch (error) {
      return { sourceId: source.id, status: 'error', records: [], error: error.message, elapsedMs: Date.now() - sourceStarted };
    }
  });
  ...
  return {
    ...
    sourcesSucceeded: perSource.filter((r) => r.status === 'success').length,
    sourcesFailed: perSource.filter((r) => r.status === 'error').length,
...
export function acquisitionCandidate(record) {
  return Boolean(record.pdfUrl) && record.rightsStatus !== 'restricted';
}
