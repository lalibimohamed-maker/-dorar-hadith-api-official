...
function normalizeRecord(record, source) {
  return {
    ...
    provenance: {
      ...
      ...(record.categoryId == null ? {} : { categoryId: record.categoryId }),
    },
  };
}
...
function retryable(error, retryStatuses) {
  return retryStatuses.includes(Number(error?.status));
}
...
function extractLanguages(json) {
  return unwrapList(json).map((item) => {
    if (typeof item === 'string') return item;
    return item?.language ?? item?.lang ?? item?.code ?? item?.iso_code ?? item?.language_code ?? item?.lang_code ?? null;
...
function extractCategories(json) {
  return unwrapList(json).map((item) => ({
    ...
    language: item?.language ?? item?.lang ?? item?.language_code ?? null,
    title: item?.title ?? item?.name ?? item?.ar_title ?? null,
  })).filter((item) => item.id !== null && item.id !== undefined).map((item) => ({ ...item, id: String(item.id) })).filter((item, index, values) => values.findIndex((value) => value.id === item.id && value.language === item.language) === index);
}
...
function mapHadeethRecords(json, source, language, categoryId) {
  return unwrapList(json).map((record) => normalizeRecord({
    identifier: record.id,
    title: record.title,
    language: record.language ?? language,
    itemUrl: `https://hadeethenc.com/api/v1/hadeeths/one/?id=${encodeURIComponent(record.id)}&language=${encodeURIComponent(record.language ?? language)}`,
    rightsUrl: 'https://hadeethenc-content.islamcontent.com/en/developers_api',
    rightsStatus: 'publisher-declared',
    method: 'hadeethenc-developer-api-v1-full-discovery',
    categoryId,
  }, source));
}
...
async function discoverHadeethEnc(source, options = {}) {
  ...
  const telemetry = [];
  const allRecords = [];
  const languagesResponse = await fetchJsonWithRetry(discovery.languagesUrl, {
    ...options,
    timeoutMs: options.timeoutMs ?? DEFAULT_TIMEOUT_MS,
    maxRetries: discovery.maxRetries,
    retryStatuses: discovery.retryStatuses,
  });
  ...
  for (const language of languages) {
    const categoryUrl = withQuery(discovery.categoriesUrl, { language });
    let categories = [];
    try {
      const categoriesResponse = await fetchJsonWithRetry(categoryUrl, {
        ...options,
        maxRetries: discovery.maxRetries,
        retryStatuses: discovery.retryStatuses,
      });
      ...
      if (!categories.length) {
        ...
        const rootsResponse = await fetchJsonWithRetry(rootsUrl, {
          ...options,
          maxRetries: discovery.maxRetries,
          retryStatuses: discovery.retryStatuses,
        });
      ...
      telemetry.push({ language, stage: 'categories', status: 'error', error: error.message });
      continue;
    }
    ...
    for (const category of categories) {
      ...
      let pagesRead = 0;
      while (true) {
        const listUrl = withQuery(discovery.hadithUrl, {
          language,
          category_id: category.id,
          page,
          per_page: options.pageSize ?? discovery.pageSize,
        });
        try {
          const response = await fetchJsonWithRetry(listUrl, {
            ...options,
            maxRetries: discovery.maxRetries,
            retryStatuses: discovery.retryStatuses,
          });
          const records = mapHadeethRecords(response.json, source, language, category.id);
          ...
          telemetry.push({ language, categoryId: category.id, categoryTitle: category.title, page, status: 'error', error: error.message });
          break;
        }
      }
  ...
  for (const record of allRecords) {
    const key = `${record.language ?? ''}:${record.identifier ?? record.itemUrl}`;
    if (!deduped.has(key)) deduped.set(key, record);
  }
  return {
    sourceId: source.id,
    strategy: discovery.strategy,
    languagesChecked: languages.length,
    recordsFound: deduped.size,
...
export function createConnectorManifest(sources) {
  return sources.map((source) => ({
    ...
    endpoints: [
      ...(source.connector.endpoint ? [source.connector.endpoint] : []),
      ...(source.connector.searchUrl ? [source.connector.searchUrl] : []),
      ...(source.connector.discovery ? [source.connector.discovery.languagesUrl, source.connector.discovery.categoriesUrl, source.connector.discovery.rootsUrl, source.connector.discovery.hadithUrl, source.connector.discovery.detailUrl].filter(Boolean) : []),
      ...(source.connector.endpointTemplate ? [source.connector.endpointTemplate] : []),
    ],
    discoveryStrategy: source.connector.discovery?.strategy ?? null,
  }));
}
