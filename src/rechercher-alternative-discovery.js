...
function firstPdfFile(files = []) {
  return files.find(file => typeof file?.name === 'string' && /\.pdf$/iu.test(file.name) && !file.private);
}

export async function discoverArchiveAlternatives(record, { fetchImpl = globalThis.fetch, inspectImpl = inspectSourceUrl, limit = 20 } = {}) {
  const q = buildArchiveQuery(record);
  if (!q || typeof fetchImpl !== 'function') return [];
  const searchUrl = new URL(ENDPOINTS.INTERNET_ARCHIVE_SEARCH);
  searchUrl.searchParams.set('q', q);
  searchUrl.searchParams.set('fl[]', 'identifier,title,creator,description,license,rights,year');
  searchUrl.searchParams.set('rows', String(Math.min(Math.max(limit, 1), 50)));
  ...
  for (const candidate of candidates) {
    try {
      ...
      const judged = buildSearchRecord({
        ...
        evidence: candidate.rightsEvidence ? [{ kind: 'source-rights-metadata', text: candidate.rightsEvidence }] : []
      }, inspected?.metadata ?? {});
      ...
      if (typeof fetchImpl === 'function') {
        ...
        if (metaResponse?.ok) {
          ...
          const pdf = firstPdfFile(metadata?.files);
          if (pdf?.name) downloadUrl = `https://archive.org/download/${encodeURIComponent(candidate.identifier)}/${String(pdf.name).split('/').map(encodeURIComponent).join('/')}`;
        }
      }
      ...
      verified.push({
        ...
        rightsVerificationSource: candidate.sourceUrl,
        rightsEvidence: judged.rightsEvidence,
        editionType: judged.editionType,
        workStatus: judged.workStatus,
        editionNeedsReview: false
      });
    } catch {
      // Fail closed: one candidate failing verification does not block others.
    }
  }
  return verified;
}
...
export async function governedAlternativeDiscovery(record, options = {}) {
  return (await discoverArchiveAlternatives(record, options))
    .sort((a, b) => (b.metadataMatchScore ?? 0) - (a.metadataMatchScore ?? 0));
}
